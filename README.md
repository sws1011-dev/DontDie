...
## 핵심 역량 및 기술적 성과
### 1. 개념적 사고력 (Conceptual Thinking) — 독립적 구조 설계
단순 구현을 넘어 복잡한 시스템을 구조화하는 시야를 발휘했습니다. 플레이어, 무기, 발사체를 각각 독립된 액터로 분리 설계하여 결합도를 낮추는 **느슨한 결합(Loose Coupling)** 구조를 정립, 유지보수와 기능 확장이 용이한 아키텍처를 완성했습니다.

### 2. 유연성 (Flexibility) — 효율적인 자원 관리
2주라는 제한된 일정과 예기치 못한 변수 속에서도 프로젝트 범위를 영리하게 한정했습니다. 마감 직전 기획 변경이나 누락을 발견했을 때, 기존 코드를 유연하게 확장 및 재사용하여 판을 뒤엎지 않고도 목표한 일정을 준수하며 완성도를 높였습니다.

### 3. 분석적 사고력 (Analytical Thinking) — 정밀한 문제 해결
추가 기능이 전체 아키텍처에 미치는 인과관계를 정확히 분석했습니다. 특히 연사 속도와 오디오 싱크 간의 정합성을 분석하여 데이터 규격과 처리 시점을 정밀하게 조율함으로써, 코드의 무결성을 유지하며 사용자 경험을 개선했습니다.
...
3. 완료 후 결과물 폴더의 `Windows/DontDie.exe`를 통해 게임을 즉시 실행할 수 있습니다.
